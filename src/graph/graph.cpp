...
void Graph::addLayer(std::shared_ptr<Layer> lay) {
  if (layers_.find(lay->getID()) == layers_.end()) {
    layers_[lay->getID()] = lay;
  }
}

void Graph::addEdge(std::shared_ptr<Layer> layPrev,
...
void Graph::removeEdge(std::shared_ptr<Layer> layPrev,
                       std::shared_ptr<Layer> layNext) {
  if (layers_.find(layPrev->getID()) != layers_.end()) {
...
void Graph::removeLayer(std::shared_ptr<Layer> lay) {
...
bool Graph::bfs_helper(int start, int vert, bool flag,
  ...
  while (!queue.empty()) {
    ...
    if (layers_.count(current) > 0) {
      std::shared_ptr<Layer> current_layer = layers_.at(current);

      for (const std::shared_ptr<Layer>& neighbor : current_layer->neighbors_) {
...
bool Graph::hasPath(std::shared_ptr<Layer> layPrev,
...
void Graph::setInput(std::shared_ptr<Layer> lay, Tensor<double>& vec) {
  if (start_ != -1) {
...
void Graph::setOutput(std::shared_ptr<Layer> lay, Tensor<double>& vec) {
  if (end_ != -1) {
...
void Graph::inference() {
  ...
  for (int layer_id : traversal) {
    ...
    std::shared_ptr<Layer> current_layer = layers_[layer_id];
